<x-app-layout>
                    ...
                    <div class="mb-6">
                        ...
                        <form id="calendar-form" action="{{ route('admin.calendar') }}" method="get" class="flex flex-col sm:flex-row sm:items-center sm:justify-center space-y-4 sm:space-y-0 sm:space-x-4">
                            ...
                            <div class="flex items-center">
                                ...
                                <select id="year" name="year" class="form-select rounded-md shadow-sm mt-1 block w-full">
                            ...
                            <tbody>
                                @foreach (array_chunk($calendarDays, 7) as $week)
                                    <tr class="text-center">
                                        @foreach ($week as $day)
                                            <td class="border border-gray-200 dark:border-gray-600 p-1 h-24 sm:h-32 align-top {{ !$day ? 'bg-gray-50 dark:bg-gray-800' : '' }}">
                                                @if ($day)
                                                    @php
                                                    ...
                                                    <div class="flex flex-col h-full">
                                                        <div class="text-sm font-semibold mb-1 {{ $isToday ? 'bg-blue-500 text-white rounded-full w-6 h-6 flex items-center justify-center mx-auto' : 'text-gray-700 dark:text-gray-300' }}">
                                                            {{ $day->format('j') }}
                                                        </div>
                                                        @if ($dataForDay)
                                                            <div class="flex-grow flex flex-col justify-center text-xs">
                                                                <div class="text-gray-600 dark:text-gray-400">Total: {{ $dataForDay['total_chicken'] }}</div>
                                                                <div class="text-green-600 dark:text-green-400">Healthy: {{ $dataForDay['total_healthy_chicken'] }}</div>
                                                                <div class="text-red-600 dark:text-red-400">Unhealthy: {{ $dataForDay['total_unhealthy_chicken'] }}</div>
                                                                <div class="text-gray-500 dark:text-gray-500">Unknown: {{ $dataForDay['total_unknown_chicken'] }}</div>
                                                            </div>
                                                        @endif
                                                    </div>
                                                @endif
    ...
    <script >
        document.getElementById('month').addEventListener('change', () => document.getElementById('calendar-form').submit());
